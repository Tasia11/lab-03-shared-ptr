// Copyright 2020 Osipova Anastasiya anastasiyaosipova2001@gmai.com
...
TEST(SharedPtr, CopyConstructor) {
  const SharedPtr<std::string> r(new std::string{"Anastasiya"});
  SharedPtr<std::string> p(r);
  ASSERT_EQ(*p, "Anastasiya");
  ASSERT_EQ(p.use_count(), 2);
}
TEST(SharedPtr, MoveConstructor) {
  SharedPtr<std::string> r(new std::string{"Osipova"});
